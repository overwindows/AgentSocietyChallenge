...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run baseline recommendation agent for Track 2 scenarios")
    parser.add_argument("--task-type", dest="task_type", choices=["classic", "item_cold_start", "user_cold_start", "long_term", "short_term"], default="classic",
                        help="Which dataset track to run (default: classic)")
    parser.add_argument("--task-set", dest="task_set", choices=["amazon", "goodreads", "yelp"], default="amazon",
                        help="Which task type to run (default: amazon)")
    parser.add_argument("--data-dir", dest="data_dir", default=DATA_DIR,
                        help="Path to processed data directory (default is DATA_DIR in script)")
    parser.add_argument("--cache", dest="cache", action="store_true", help="Enable interaction tool cache (default: False)")
    parser.add_argument("--max-workers", dest="max_workers", type=int, default=10,
                        help="Max workers for threaded simulation (default: 10)")
    ...
    simulator = Simulator(
        data_dir=args.data_dir, device="auto", cache=args.cache)

    # Load scenarios
    simulator.set_task_and_groundtruth(
        task_dir=f"/mnt/q/AgentRecBench/task/{task_type}/{task_set}/tasks", 
        groundtruth_dir=f"/mnt/q/AgentRecBench/task/{task_type}/{task_set}/groundtruth")
    ...
    agent_outputs = simulator.run_simulation(
        number_of_tasks=None, enable_threading=True, max_workers=args.max_workers)

    # Evaluate the agent
    evaluation_results = simulator.evaluate()
    with open(f'./evaluation_results_{task_type}_{task_set}.json', 'w') as f:
